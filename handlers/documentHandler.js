...
export async function postDocumentHandler(ctx, state) {
...
export async function storyDocumentHandlerUp(ctx, state) {
    if (state.interactionStage !== 'waitingForFile') {
        ctx.reply('Сначала завершите предыдущий процесс или введите /start для новой обработки.');
        return;
    }
...
export async function storyDocumentHandlerDown(ctx, state) {
    if (state.interactionStage !== 'waitingForFile') {
        ctx.reply('Сначала завершите предыдущий процесс или введите /start для новой обработки.');
        return;
    }
